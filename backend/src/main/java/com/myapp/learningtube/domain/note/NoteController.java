...
@Tag(name = "Notes", description = "노트 id 기준 수정·삭제")
...
public class NoteController {
    ...
    @PatchMapping("/{noteId}")
    @Operation(summary = "노트 수정", description = "소유자만. 타입·positionSec 변경 시 규칙 재검증.")
    @ApiResponses({
    ...
    public ApiSuccessResponse<NoteResponse> patch(
            @AuthenticationPrincipal CustomUserPrincipal principal,
            @PathVariable Long noteId,
            @Valid @RequestBody UpdateNoteRequest body,
            HttpServletRequest request) {
        NoteResponse data = noteService.update(principal.getId(), noteId, body);
        return ApiSuccessResponse.ok(resolveRequestId(request), data);
    }

    @DeleteMapping("/{noteId}")
    @Operation(summary = "노트 삭제", description = "소유자만")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "성공, data null"),
        @ApiResponse(
                responseCode = "404",
                description = "노트 없음",
                content = @Content(schema = @Schema(implementation = ApiErrorResponse.class)))
    ...
    public ApiSuccessResponse<Void> delete(
            ...
            @PathVariable Long noteId,
            HttpServletRequest request) {
        noteService.delete(principal.getId(), noteId);
